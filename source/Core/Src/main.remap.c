...
void* FP_REMAP_TABLE[6] = {(void *)&return_zero};


void setup_fpb(void)
{

  // set remap table in RAM
  ...
  *FP_REMAP = (uint32_t)FP_REMAP_TABLE;
  ...
  comp_value |= (uint32_t)&return_one; // Set comparison address
  comp_value |= 1;		       // Enable comparator
  *FP_COMP0 = comp_value;
  //
  // Enable FPB
